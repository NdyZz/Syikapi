...
async function updateHit() {
  let visit = await Visitor.findOne({id: "visitors"})
  if (visit) {
    visit.hit++;
    await visit.save();
    return visit;
  } else {
    const newVisitor = new Visitor({ id: "visitors" });
    await newVisitor.save();
    return newVisitor;
  }
}
module.exports.updateHit = updateHit;

async function getVisitor() {
    try {
        const visitor = await Visitor.findOne({ id: "visitors" });
        if (visitor == null) {
          await updateHit()
          return 1
        } else {
