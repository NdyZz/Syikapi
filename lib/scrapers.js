...
module.exports = {
    ...
    XVideosDL: async (url) => {
      ...
      .request(url, {
        ...
        headers: {
          ...
          'Sec-Ch-Ua-Mobile': '?1',
          'Sec-Ch-Ua-Model': '"CPH2641"',
          'Sec-Ch-Ua-Platform': '"Android"',
          'Sec-Ch-Ua-Platform-Version': '"15.0.0"',
          'Viewport-Width': 980,
          'cookie': 'session_token=9ab6bb6a9dccc36eAB-G4Hj2N4oF90g4a3TVmuMraugkHQSYy9Gmg_8eVIG-9dlwFKbnxWB7gbzgHcclZugpyT9307_0f42AbpdHghXnc_myNnB0Tjv8axeQgtuqOWGruz-h-LqYiULnakm0bcDM-WvKVMaJjikvY5KIpm3fypmu7yN1N7HmvVRj8HdbfPpWduXJLSUNzq-Yjuy4WqYyux7K5N8TPseOGIShJw%3D%3D'
        },
      })
      .then((res) => {
        const $ = cheerio.load(res.data);
        results.low = $("a:contains('View Low Qual')").attr("href");
        results.high = $("a:contains('View High Qual')").attr("href");
      });
      return results;
    },
    pornHubDL: async (uri) => {
      ...
      .request(uri, {
        ...
        headers: {
          ...
          'Sec-Ch-Ua-Platform': '"Android"',
          'Sec-Ch-Ua-Platform-Version': '"15.0.0"',
          'cookie': 'ua=b837718ffb5a3b4b1d9498234a1f1b0b; platform=mobile; bs=000000000000000008860fb523a20a61; bsdd=000000000000000008860fb523a20a61; ss=784857420694319222; sessid=321518331751685770; comp_detect-cookies=83213.100000; fg_afaf12e314c5419a855ddc0bf120670f=63129.100000; fg_7d31324eedb583147b6dcbea0051c868=12613.100000; __s=690163DC-42FE722901BB8CC7C-70B01CE; __l=690163DC-42FE722901BB8CC7C-70B01CE; tj_UUID_v2=ChClhYi8o2VFRK7VWcuKHE9jEgwI3seFyAYQrNy1rAIYAyoMCN7HhcgGEKyehLECMgaog4CAgHY6CavyjNjmxMjMKw==; _ga=GA1.1.1290966079.1761698784; d_fs=1; d_uidb=60def207-8a2d-a09a-0ac3-eabbaedb8d6e; accessAgeDisclaimerPH=2; d_uid=60def207-8a2d-a09a-0ac3-eabbaedb8d6e; d_uidb=60def207-8a2d-a09a-0ac3-eabbaedb8d6e; cookieConsent=3; fg_439f2555043a44b8bd91161b5deddd29=44085.100000; htjf-mobile=2; _ga_B39RFFWGYY=GS2.1.s1761698783$o1$g1$t1761698816$j27$l0$h0; g_state={"i_l":0,"i_ll":1761698818335,"i_b":"8HZNFS5+BVyt+ApE4MHutJFDtaooJOD4AR4e6DK2HrU"}; tj_UUID=ChClhYi8o2VFRK7VWcuKHE9jEgwI3seFyAYQrNy1rAIYAyoLCPLJhcgGELPAp14yBqiDgICAdjobq_KM2ObEyMwrmdDjpbjtyOl-2sirwsOVrMgl',
        },
      })
      .then((res) => {
        const $ = cheerio.load(res.data);
        const scripts = $('div.notPremium > script').filter(function() {
          return $(this).html().includes('flashvars');
        }).html();

        const regex = /var flashvars.*?({.*})/s;
        const match = scripts.match(regex);
        const flashvars = JSON.parse(match[1].split(';')[0]);

        const mediaDefinitions = flashvars.mediaDefinitions;
        const videoUrls = mediaDefinitions.map(definition => ({
          quality: definition.quality,
          videoUrl: definition.videoUrl
        }));
        results.results = videoUrls;
      });
      return results;
    },
    xHamsterDL: async (uri) => {
      let results = {}
      await instance
      .request(uri, {
        method: "GET",
        ...
        headers: {
          ...
          'Sec-Ch-Ua-Mobile': '?1',
          'Sec-Ch-Ua-Platform': '"Android"',
          'cookie': 'cookie_accept_v2=%7B%22e%22%3A1%2C%22f%22%3A1%2C%22t%22%3A1%2C%22a%22%3A1%7D; _cfg=cf6434b2f6aada16161d0f98cdc33496; ss=%7B%22domainAccessibility%22%3Atrue%7D; parental-control=yes; ff_thumb_offset=2; recs_show_time=1761731548; moments_listing_ad_offset=3; settings=; last_video_search=step%20daughter; search_last_list=%5B%22step%20doughter%22%5D; video_view_count=0; g_state={"i_l":0,"i_ll":1761731553879,"i_b":"IIoRD5BTsVEslyrcsZgqqJg9zE3BKwnmXx8eWnKyUL8"}',
        },
      })
      .then(async (res) => {
        const $ = await cheerio.load(res.data)
        const src = $('.video_container').filter(function() {
  ...
  BPSearch: {
    searchXVideos: async (querry) => {
      const BASE_URL = `https://www.xvideos.com/?k=${querry}`
      let results = [];
      await instance
      .request(BASE_URL, {
        method: "GET",
      ...
      .then((res) => {
        ...
        $(".frame-block.thumb-block").each((i, el) => {
          const element = $(el);
          const href = element.find(".thumb a").attr("href")?.trim() || "";
          const img = element.find(".thumb a img").attr("data-src")?.trim() || "";
          ...
          const duration = titleElement.find(".duration").text().trim();
          const name = element.find(".metadata .name").text().trim();
          const views =
          element
          .find(".metadata")
          .text()
          .match(/([\d.]+[KMB]?)\s*Views/i)?.[1] || "";
    ...
    searchPornHub: async (query) => {
      let results = [];
      const BASE_URL = `https://www.pornhub.com/video/search?search=${query}`
      await instance
      .request(BASE_URL, {
        method: "GET",
        url: BASE_URL,
      ...
      .then((res) => {
        const $ = cheerio.load(res.data);
        $(".videoList > li").each(function(i, el) {
          const url = $(el).find('a').attr('href');
          const views = $(el).find('.rightAlignInfo > .videoViews').text().trim();
          const gif = $(el).find('a').attr('data-webm');
          const img = $(el).find('img').attr('src');
          const title = $(el).find('img').attr('alt') || $(el).find('.vidTitleWrapper > .title').text().trim();
          results.push({
    ...
    searchXHamster: async (query) => {
      let results = [];
      const BASE_URL = `https://xhamster.com/search/${new URLSearchParams(query)}`
      await axios
      .request(BASE_URL, {
        method: "GET",
        url: BASE_URL,
      ...
      .then((res) => {
        const $ = cheerio.load(res.data);
        $(".search-results > div > div > ul > li:lt(5)").each(function(i, el) {
          const url = $(el).find('a').attr('href');
          const views = $(el).find('.video-thumb-views').text().trim();
          const gif = $(el).find('a').attr('data-previewvideo');
          const img = $(el).find('a.video-uploader-logo').attr('data-background-image');
          const title = $(el).find('img').attr('alt') || $(el).find('.vidTitleWrapper > .title').text().trim();
          results.push({
            url: new URL(url, BASE_URL).href,
            views,
  ...
  BibleAI,
  film: async (query, page = 1) => {
  const response = {results:[]};
  await axios.get(`https://api.themoviedb.org/3/search/multi?query=${new URLSearchParams(query)}&include_adult=false&language=en-US&page=${page}`,{
    headers: {
    ...
    const {results, total_pages, total_results} = await data;
    ...
    for (let res of results) {
      response.results.push({
        ...
        dec: res.overview || '',
        poster: res.poster_path ? `https://image.tmdb.org/t/p/original/${res.poster_path}?w=828&q=70`:'',
        bg: res.backdrop_path ? `https://image.tmdb.org/t/p/original/${res.backdrop_path}?w=828&q=70`:'',
        release_date: res.release_date || '',
        url_vidplus: `https://player.vidplus.to/embed/${res.media_type}/${res.id}?autoplay=true`,
        url_donkey: `https://donkey.to/watch-now?type=${res.media_type}&id=${res.id}&seasonId=1&episodeId=1`,
